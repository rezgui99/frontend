...
const applyToJobOffer = async (req, res) => {
  ...
  try {
    ...
    console.log('👤 Candidate ID:', req.candidate?.id);
    ...
    if (!cv_id) {
      await t.rollback();
      return res.status(400).json({ error: 'CV requis pour postuler' });
    }

    if (!cover_letter || cover_letter.trim().length < 50) {
      await t.rollback();
      return res.status(400).json({ error: 'Lettre de motivation requise (minimum 50 caractères)' });
    }
    ...
    if (!cv) {
      await t.rollback();
      return res.status(404).json({ error: 'CV non trouvé ou ne vous appartient pas' });
    }
    ...
    console.log('✅ Application created:', {
      ...
      cover_letter_length: application.cover_letter?.length,
      ...
      await transporter.sendMail({
        ...
        html: `
          ...
          <ul>
            ...
            <li><strong>CV utilisé :</strong> ${cv.title}</li>
          </ul>
    ...
    console.log('✅ Full application created with relations:', {
      id: fullApplication.id,
      jobOffer: fullApplication.jobOffer?.title,
      cv: fullApplication.cv?.title,
      cover_letter_present: !!fullApplication.cover_letter,
      proposed_slots_count: fullApplication.proposed_interview_slots?.length || 0
    });

    // Créer automatiquement un entretien pour cette candidature
    try {
      const interview = await createAutomaticInterview(fullApplication, t);
      console.log('✅ Automatic interview created with ID:', interview.id);
    } catch (interviewError) {
      console.error('⚠️ Warning: Could not create automatic interview:', interviewError.message);
      // Ne pas faire échouer la candidature si l'entretien ne peut pas être créé
    }
...
const createAutomaticInterview = async (application, transaction) => {
  try {
    ...
    // Calculer une date automatique pour l'entretien
    const minDays = 3;
    const maxDays = 7;
...
const getCandidateApplications = async (req, res) => {
  try {
    console.log('📋 Getting applications for candidate:', req.candidate?.id);
...
const sendInterviewNotificationToRecruiter = async (application, interview, interviewer) => {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #2196F3;">🤖 Entretien automatique créé</h1>
          <p>Bonjour ${interviewer.firstName} ${interviewer.lastName},</p>
          
          <div style="background-color: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <h3>Un entretien automatique a été créé suite à une nouvelle candidature :</h3>
            <ul>
              ...
              <li><strong>Type :</strong> ${getInterviewTypeLabel(interview.interview_type)}</li>
              <li><strong>Lien Meet :</strong> <a href="${interview.meeting_link}">${interview.meeting_link}</a></li>
              <li><strong>Statut :</strong> En attente de votre confirmation</li>
            </ul>
          </div>
          
          <div style="background-color: #fef3c7; padding: 15px; border-radius: 8px; margin: 20px 0;">
            <h4 style="color: #92400e; margin-top: 0;">⚠️ Action requise :</h4>
            <p style="color: #78350f;">
              Cet entretien a été créé automatiquement. Veuillez vous connecter au backoffice pour :
            </p>
            <ul style="color: #78350f; font-size: 14px;">
              <li>Confirmer ou modifier la date/heure</li>
              <li>Choisir le mode d'entretien définitif</li>
              <li>Envoyer la confirmation au candidat</li>
            </ul>
          </div>
          
          <div style="text-align: center; margin: 20px 0;">
            <a href="${process.env.FRONTEND_URL}/recruiter/interviews" 
               style="background-color: #2196F3; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
    ...
    console.log('✅ Automatic interview notification email sent to recruiter');
  } catch (emailError) {
